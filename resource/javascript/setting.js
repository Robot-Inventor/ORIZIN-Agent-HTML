...
document.getElementById("check_update").addEventListener("click", async function () {
    ...
    if (latest_version === version_information) {
        ...
        update_detail.innerHTML = `<h1>${release_data[0]}</h1>${release_data[1]}<br><a href='https://github.com/Robot-Inventor/ORIZIN-Agent-HTML/releases/tag/${latest_version}' target='_blank' rel='noreferrer noopener' class='ripple_effect'>ダウンロード<i class='material_icon'>open_in_new</i></a>`;
    }
});
