...
- ライトテーマの背景色を少し暗く変更し，カードの背景色と区別しやすくします。
- ヘッダーの「ORIZIN Agent」という文字のフォントをUIのフォントと同じにし，「ORIZIN Agent HTML」に変更します。
- 「カスタムCSSテーマの設定」ページの名称を「カスタムテーマの編集」に変更し，デザインも変更します。
- 「カスタムテーマの編集」ページのカラーピッカーのプリセットの不透明度をすべて100%に変更します。
- 音声に関する設定を正式な機能に移行し，「音声に関する設定を追加する」試験機能を削除します。
- 処理の簡略化のため，トグルスイッチと下線付きテキストボックスをコンポーネント化します。
- v4.0になったため，スプラッシュスクリーンのローディングアニメーションを変更しました。
- 設定画面でのテーマの一覧の生成方法を変更し，ユーザーがテーマを追加しやすくします。
- 開発モード時のログのフォーマットを変更します。

#### 追加
- 豆知識機能を追加します。
- 今日は何の日機能を追加します。
- ホーム画面のテキストボックスにカーソルがある時，上下矢印キーで履歴を呼び出せる機能を追加します。
- メニューボタンクリック時にメニューアイコンが回転するアニメーションを追加します。
- メニュー内にクリックでメニューを閉じられるボタンを追加します。
- メニューの項目に「ホーム」を追加します。
- テーマを7種類追加します。
- コマンドライン引数で-Cまたは--cui_modeを指定すると有効にできる低スペックのPC向けのCUIモードを追加します。
- 不適切な可能性がある単語をブロックする機能を追加します。

#### 削除
- アップデートチェッカーを完全に削除します。

### [v4.0.123.0dev-Crane] - 2020-09-01
#### 追加
...
#### 追加
- 11通りの応答を辞書に追加し，応答は全てで1819通りになりました。
...
- 13通りの応答を辞書に追加し，応答は全てで1804通りになりました。
...
#### 追加
- 10通りの応答を辞書に追加し，応答は全てで1791通りになりました。
...
#### 追加
- 13通りの応答を辞書に追加し，応答は全てで1781通りになりました。
...
#### 追加
- 20通りの応答を辞書に追加し，応答は全てで1768通りになりました。
...
#### 追加
- 8通りの応答を辞書に追加し，応答は全てで1776通りになりました。
...
#### 追加
- 12通りの応答を辞書に追加し，応答は全てで1768通りになりました。
...
- 30通りの応答を辞書に追加し，応答は全てで1756通りになりました。
...
- ホーム画面でエンターキーでテキストボックスの内容を送信できる機能を追加しました。
- 11通りの応答を辞書に追加し，応答は全てで1726通りになりました。
...
- 新型コロナウイルス接触確認アプリ（COCOA）に関する厚生労働省のページを開く機能を追加しました。
- ホーム画面の新型コロナウイルス対策を呼びかけるメッセージに「新型コロナウイルスに関する厚生労働省ホームページ」と「新型コロナウイルス接触確認アプリ（COCOA）について」のリンクを追加しました。
- 5通りの応答を辞書に追加し，応答は全てで1720通りになりました。
- 音声合成中は音声認識ボタンを押せない仕様がわかりやすいように色が変わるようになりました。
- ホーム画面でエンターキーでテキストボックスの内容を送信できる機能を追加しました。
...
- 13通りの応答を辞書に追加し，応答は全てで1715通りになりました。
...
- 61通りの応答を辞書に追加し，応答は全てで1702通りになりました。
...
#### 追加
- 22通りの応答を辞書に追加し，応答は全てで1702通りになりました。
...
#### 追加
- blackouttuesday，ブラックアウトチューズデー，blacklivesmatter，ブラックリブズマター，blmなどと入力すると黒い画面が表示されるようになりました。
- 39通りの応答を辞書に追加し，応答は全てで1682通りになりました。
...
#### 追加
- 106通りの応答を辞書に追加し，応答は全てで1643通りになりました。
...
#### 追加
- 86通りの応答を辞書に追加し，応答は全てで1643通りになりました。
- 音声認識の処理中に音声認識を中断する機能を追加しました。
- 連続で音声認識をする試験機能を追加しました。
