...
class Otfd:
    ...
    str_or_list_that_contains_str = typing.TypeVar(
        "str_or_list_that_contains_str", str, list)
    ...
    def unescape(target: str_or_list_that_contains_str) -> str_or_list_that_contains_str:
        if type(target) is str:
            ...
            result = [str(string).replace("&#47", ":").replace(
                "&#58", "/") for string in target]
        else:
            result = str(target)
        return result
    ...
    def escape(target: str_or_list_that_contains_str) -> str_or_list_that_contains_str:
        if type(target) is str:
            ...
            result = [string.replace(":", "&#47").replace(
                "/", "&#58") for string in target]
        else:
            result = str(target)
        return result
    ...
    def parse(self) -> OrderedDict:
        ...
        if error_place:
            raise Exception(
                "otfdファイル内の:の数と改行の数が合いません。不正な値です。問題のある個所->\n" + "\n".join(error_place))
        self.parsed_otfd = OrderedDict(
            [line.split(":") for line in splited_with_line])
        return self.parsed_otfd
    ...
    def to_string(self) -> str:
        ...
        data_in_list = [":".join(data_in_list[num])
                        for num in range(len(data_in_list))]
        return "\n".join(data_in_list)
