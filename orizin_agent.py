...
def make_response(_not_normalized_query):
    ...
    if core.judge(_query, ["じゃんけん", "ジャンケン"]):
        ...
        return ["ジャンケンですか。良いですね。やりましょう。それではいきますよ。ジャン　ケン　ポン。私は" + selected_hand_shape + "です。", "ジャンケンですか。良いですね。やりましょう。それではいきますよ。ジャン　ケン　ポン。私は" + selected_hand_shape + "です。"]
        ...
        webbrowser.open_new("https://www.youtube.com/results?search_query=" + _url_format_query)
        return ["YouTubeで「" + _not_normalized_query + "」を検索します。", "YouTubeで「" + _not_normalized_query + "」を検索します。"]
    ...
    elif core.judge(_query, ["メモ", "memo", "めも"]) and core.judge(_query, ["削除", "さくじょ", "消去", "しょうきょ", "クリア", "clear", "くりあ", "消し", "けし", "忘れ", "わすれ"]):
    ...
    elif core.judge(_query, ["メモ", "memo", "めも", "何", "なに", "内容", "ないよう"]) and core.judge(_query, ["覚えている", "おぼえている", "覚えてる", "おぼえてる", "教え", "おしえ", "読", "よめ", "よん", "よみ"]):
        if os.path.exists("memo.txt") is False:
            return ["覚えているメモはありません。", "覚えているメモはありません。"]
        else:
            _f = open("memo.txt", mode="r", newline="")
            ...
            if _memo.strip() == "":
                ...
                _memo_list = [str(num + 1) + "つ目は、「" + _memo_list[num] + "」です。" for num in range(len(_memo_list))]
                _memo_content_to_read = "".join(_memo_list)
                return ["覚えているメモを読み上げます。" + _memo_content_to_read, "覚えているメモを読み上げます。" + _memo_content_to_read]
    elif core.judge(_query, ["メモ", "memo", "めも", "覚えて", "おぼえて", "覚えと", "おぼえと"]):
        if os.path.exists("memo.txt") is False:
            _f = open("memo.txt", mode="w", newline="")
            _f.close()
        ...
        _f.write(_query + "\n")
        _f.close()
        return ["「" + _query + "」とメモしました。", "「" + _query + "」とメモしました。"]
    elif core.judge(_query, ["て何" ,"てなに", "意味", "とは", "教え", "おしえ", "検索", "けんさく", "調べ", "しらべ", "調査", "ちょうさ"]):
        webbrowser.open_new("https://google.com/search?q=" + _url_format_query)
        return ["Googleで「" + _not_normalized_query + "」を検索します。", "Googleで「" + _not_normalized_query + "」を検索します。"]
    elif core.judge(_query, ["サイコロ", "さいころ", "ダイス", "だいす", "dice"]):
        dice_result = random.randint(1, 6)
        return ["サイコロを振りますね。" + str(dice_result) + "が出ました。", "サイコロを振りますね。" + str(dice_result) + "が出ました。"]
    elif core.judge(_query, ["コイン", "こいん", "coin"]) and core.judge(_query, ["トス", "とす", "toss", "投げ", "なげ"]):
        coin = ["表", "裏"]
        result = coin[random.randint(0, 1)]
        return ["コイントスをしますね。" + result + "が出ました。", "コイントスをしますね。" + result + "が出ました。"]
