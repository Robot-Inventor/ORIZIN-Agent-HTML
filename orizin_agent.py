...
def add_chat(content: str) -> None:
    if IS_CUI_MODE:
        ...
        print_log_if_dev_mode(
            "Add chat at index.html by Python.", OrderedDict(Content=content))
        return
...
def start_speak(content: str) -> None:
    if IS_CUI_MODE:
        ...
        print_log_if_dev_mode(
            "Start speak at index.html by Python.", OrderedDict(Content=content))
        return
...
def make_response(not_normalized_query: str) -> list[typing.Union[str, bool]]:
    ...
    if core.judge(query, "ジャンケン"):
        ...
        subprocess.Popen([read_setting("python_interpreter"),
                          "resource/python/easter_egg.py"])
        ...
        if area_value[0] == "そうですか。":
            webbrowser.open_new(core.generate_search_engine_url(
                read_setting("search_engine"), not_normalized_query))
            response = ["面積を検索します。", "面積を検索します。", False]
        else:
            response = [f"{area_value[2]}の面積は{area_value[0]}です。",
                        f"{area_value[2]}の面積は{area_value[1]}です。"]
        print_log_if_dev_mode_template()
        return response
        ...
        if capital_name[0] == "そうですか。":
            ...
            response = [f"{capital_name[2]}の首都は{capital_name[0]}です。",
                        f"{capital_name[2]}の首都は{capital_name[1]}です。"]
        print_log_if_dev_mode_template()
        return response
        ...
        if language[0] == "そうですか。":
            ...
            response = [f"{language[2]}の言語は{language[0]}です。",
                        f"{language[2]}の言語は{language[1]}です。"]
        print_log_if_dev_mode_template()
        return response
        ...
        if population[0] == "そうですか。":
            ...
            response = [f"{population[2]}の人口は{population[0]}です。",
                        f"{population[2]}の人口は{population[1]}です。"]
        print_log_if_dev_mode_template()
        return response
        ...
        if religion[0] == "そうですか。":
            ...
            response = [f"{religion[2]}の宗教は{religion[0]}です。",
                        f"{religion[2]}の宗教は{religion[1]}です。"]
        print_log_if_dev_mode_template()
        return response
        ...
        webbrowser.open_new("https://google.com/maps/search/" +
                            urllib.parse.quote(not_normalized_query))
        response = [f"Googleマップで「{not_normalized_query}」を検索します。",
                    f"Googleマップで「{not_normalized_query}」を検索します。", False]
        print_log_if_dev_mode_template()
        return response
        ...
        if core.judge(unicodedata.normalize("NFKC", not_normalized_query.replace(" ", "").lower()), [
            ...
            webbrowser.open_new(
                f"https://twitter.com/intent/tweet?text={tweet_content}")
            response = [
        ...
        response = ["新型コロナウイルス接触確認アプリ（ココア）についての厚生労働省のページを開きます。",
                    "新型コロナウイルス接触確認アプリ（COCOA）についての厚生労働省のページを開きます。", False]
        print_log_if_dev_mode_template()
        return response

    elif core.judge(query, ["コロナ", "corona", "covid19", "sarscov2", "(感|カン)(染|セン)", "(肺|ハイ)(炎|エン)"]):
        webbrowser.open_new("https://corona.go.jp/")
        response = ["内閣官房の新型コロナウイルスに関するページを表示します。",
                    "内閣官房の新型コロナウイルスに関するページを表示します。", False]
        print_log_if_dev_mode_template()
        return response
        ...
        response = [f"YouTubeで「{not_normalized_query}」を検索します。",
                    f"YouTubeで「{not_normalized_query}」を検索します。", False]
        print_log_if_dev_mode_template()
        return response
        ...
        webbrowser.open_new(
            "https://trends.google.com/trends/trendingsearches/daily?geo=JP")
        ...
        if core.judge(unicodedata.normalize("NFKC", not_normalized_query.replace(" ", "").lower()), [
            ...
            webbrowser.open_new(
                f"https://mail.google.com/mail/?view=cm&body={message_content}")
            response = [f"「{message_content}」という内容でGmailの送信画面を開きます",
                        f"「{message_content}」という内容でGmailの送信画面を開きます", False]
        else:
            ...
            if memo.strip() == "":
                ...
                if core.judge(query, [r"\d+?((ツ|個|コ|番|バン)(|メ|目))"]):
                    ...
                    memo_index = int(re.search(
                        r"\d+?", (re.search(r"\d+?((ツ|個|コ|番|バン)(|メ|目))",
                                            query).group())
                    ).group())
                    if memo_index > len(splited_memo) or memo_index < 1:
                        response = ["削除するメモの番号が正しくありません。",
                                    "削除するメモの番号が正しくありません。"]
                    else:
        ...
        if os.path.exists("memo.txt") is False:
            ...
            if memo.strip() == "":
                ...
                memo_list = [
                    f"{num + 1}つ目は、「{memo_list[num]}」です。" for num in range(len(memo_list))]
        ...
        if memo_content == "":
            ...
            response = [f"「{memo_content}」とメモしました。",
                        f"「{memo_content}」とメモしました。"]
        print_log_if_dev_mode_template()
        return response
        ...
        if core.judge(query, ["一昨[々昨]日", "サキオトトイ"]):
            ...
            day_point = "明々後日"
        webbrowser.open_new(
            f"https://ja.wikipedia.org/wiki/Wikipedia:今日は何の日_{now.month}月#{now.month}月{now.day}日")
        response = [f"Wikipediaの「今日は何の日」の{day_point}のページを開きます。",
                    f"Wikipediaの「今日は何の日」の{day_point}のページを開きます。"]
        print_log_if_dev_mode_template()
        return response
        ...
        with open("resource/dictionary/trivia_dictionary.txt", encoding="utf-8_sig") as trivia_file:
            ...
            response = [f"{user_name}さん、知っていましたか？{chosen_trivia[0]}",
                        f"{user_name}さん、知っていましたか？{chosen_trivia[1]}"]
            print_log_if_dev_mode_template()
            return response
        ...
        if core.judge(query, [r"\d(面|メン)"]):
            max_number = re.sub("(面|メン)", "", re.search(
                r"\d*(面|メン)", query).group())
        if int(max_number) <= 0:
            max_number = 6
        ...
        result = fortune_repertoire[random.randint(
            0, len(fortune_repertoire) - 1)]
        response = [
            f"おみくじをします。ガラガラ...。結果は・・・{result}です。", f"おみくじをします。ガラガラ...。結果は・・・{result}です。"]
        print_log_if_dev_mode_template()
        return response

    elif core.judge(query, list(YOUTUBE_MUSIC_VIDEOS.keys())):
        music_data = YOUTUBE_MUSIC_VIDEOS[core.judge(
            query, list(YOUTUBE_MUSIC_VIDEOS.keys()), True)[1]]
        if IS_CUI_MODE is False:
            eel.add_chat(music_data[4], False, True)
        response = [f"{music_data[3]}の{music_data[1]}です。",
                    f"{music_data[2]}の{music_data[0]}です。"]
        print_log_if_dev_mode_template()
        return response
...
def set_intelligent_timer(query: str) -> str:
    ...
    if "時間" in query:
        hours = int(
            re.search(r'\d*', re.search(r'\d*時間', query).group()).group())
    if "分" in query:
        minutes = int(
            re.search(r'\d*', re.search(r'\d*分', query).group()).group())
    if "秒" in query:
        seconds = int(
            re.search(r'\d*', re.search(r'\d*秒', query).group()).group())
    time = ""
    if hours:
        time = f"{hours}時間"
    if minutes:
    ...
    if not time:
        time = "0秒"
    subprocess.Popen(
        f"{read_setting('python_interpreter')} timer.py {hours} {minutes} {seconds}")
    print_log_if_dev_mode("Start timer app.", OrderedDict(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="オープンソースの会話を目的としたAIアシスタント「ORIZIN Agent HTML」です。")
